    ...
    @foreach($sppnalozi as $nalog)
        <tr>
            ...
            <td>
                ...
                <a class="btn btn-xs btn-info" title="kreiraj nalog" href="{{ URL::to('slucaj/'.$nalog->id.'/edit') }}"><i class="glyphicon glyphicon-export"></i></a>
                @if ($adminUser) 
                    <a class="btn btn-xs btn-danger confirmDelete" data-placement="left" title="Briši nalog?" data-delete="{{csrf_token()}}" data-dest="/SPPnalozi" data-myhref="{{ URL::to('slucaj/'.$nalog->id)}}"><i class="glyphicon glyphicon-trash"></i></a>
                @endif
